...
蓋を閉じたときのスリープを、信頼できるネットワークかつ AC 接続のときだけ止める LaunchDaemon。
...
以下の AND を満たすときだけ `disablesleep 1`、外れたら `0` に戻す。

- AC 電源に接続中
- 物理ゲートウェイの MAC アドレスが設定ファイルの `KNOWN_GATEWAYS` に含まれる

30 秒間隔で評価するので、電源を抜いた直後やネットワークを移った直後に自動で復帰する。

設定が空、または設定ファイルそのものが無い場合はどのネットワークも既知と見なさず、抑止は一度も有効にならない。設定漏れが抑止の垂れ流しにならない側へ倒してある。
